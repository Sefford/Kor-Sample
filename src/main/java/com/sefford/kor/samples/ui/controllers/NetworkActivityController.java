 ...
 */
public class NetworkActivityController implements NetworkActivityControllerInterface {
    ...
    @Inject
    public NetworkActivityController(BusControllerInterface bus, Builder builder, DataExecutor provider, InteractorFactory factory) {
        this.bus = bus;
        ...
        onRefreshListener = new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                if (!srlRefresh.isRefreshing()) {
                    ...
                    provider.executeOperation(factory.generateInteractor(InteractorCodes.GET_USERS,
                            Options.create().setParams("40")
                                    .create()));
                }
            }
        };
    ...
    public void onEventMainThread(GetUserResponse response) {
        if (response.isSuccess()) {
            devices.addAll(response.getUserList());
            updateAdapter();
        }
        srlRefresh.setRefreshing(false);
    }

    public void onEventMainThread(GetUserError error) {
        srlRefresh.setRefreshing(false);
    }
}
